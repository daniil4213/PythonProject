...
running = True
while running:
	win.blit(pygame.image.load('fon.png'), (0, 0))
	...
	pygame.draw.rect(win, Yellow, (0, HEIGHT-120, WIDTH, 120))
	if tetris.next:
		for i in range(4):
			for j in range(4):
	...
	scoreimg = font.render(f'{tetris.score}', True, BLACK)
	levelimg = font2.render(f'Level : {tetris.level}', True, BLACK)
	win.blit(scoreimg, (250-scoreimg.get_width()//2, HEIGHT-110))
	win.blit(levelimg, (250-levelimg.get_width()//2, HEIGHT-30))

	pygame.draw.rect(win, Yellow, (0, 0, WIDTH, HEIGHT-120), 2)
	clock.tick(FPS)
